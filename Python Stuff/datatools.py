...
class DataTools(object):
    ...
    def send_to_rtview(self, table, cacheName):
        ...
        try:
            ...
            response = requests.post(self.g_rtvUrlWrite + cacheName, data=json.dumps(table), headers=headers)
    ...
    def command_to_rtview(self, table, command, cacheName):
        ...
        try:
            ...
            #print('    ... last send time: ', self.lastSendTime)
            headers = {'content-type': 'application/json'}
            response = requests.post(self.g_rtvUrlCommand + command + '/' + cacheName, data=json.dumps(table), headers=headers)
            # TODO: check return code before declaring success
            #print('... response: ' + str(response))
            return True
